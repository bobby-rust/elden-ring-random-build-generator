...
function App() {
  ...
  // Find incantations
  const rand_incant_idx_1 = Math.floor(Math.random() * incantations_data.count)
  ...
  return (
    <div className="App">
      ...
      <div className="weapons-container">
        <Item name={rand_weapon_1.name} img={rand_weapon_1.image} />
        <div className="weapon-desc-container">
          <p className="weapon-desc">{rand_weapon_1.description}</p>
        </div>
        <Item name={rand_weapon_2.name} img={rand_weapon_2.image} />
        <div className="weapon-desc-container">
          <p className="weapon-desc">{rand_weapon_2.description}</p>
        </div>
      </div> 
      <div className="armor-pieces-container">
        {armors_found.map((armor, idx) => {
          console.log(armor)
          return( <Item name={armor.name} img={armor.image} />
        )})}
      </div>
      <div className="class-container">
          <Item name={starting_class.name} img={starting_class.image} />
      </div>
      <div className="shield-container">
        <Item name={shield.name} img={shield.image} />
      </div>
      <div className="sorc-container">
        <Item name={sorc_1.name} img={sorc_1.image} />
        <Item name={sorc_2.name} img={sorc_2.image} />
      </div>
      <div className="incant-container">
        <Item name={incant_1.name} img={incant_1.image} />
        <Item name={incant_2.name} img={incant_2.image} />
      </div>
      <div className="talismans-container">
        <Item name={talisman_1.name} img={talisman_1.image} />
        <Item name={talisman_2.name} img={talisman_2.image} />
      </div>
      <div className="ashes-container" >
        <Item name={ashes.name} img={ashes.image} />
      </div>
      <div className="spirits-container">
        <Item name={spirit.name} img={spirit.image} />
      </div>
    </div>
  );
}
